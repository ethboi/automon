...
export default function CrystalCaves() {
  ...
  return (
    <group>
      ...
      {/* Side rock mass — left */}
      <mesh position={[-3, 0.3, 1]} geometry={sideRock1} castShadow>
        <meshStandardMaterial color="#33333b" roughness={0.95} flatShading />
      </mesh>

      {/* Side rock mass — right back */}
      <mesh position={[2.5, 0.3, -2]} geometry={sideRock2} castShadow>
      ...
      {/* 5 large crystals */}
      {[
        { pos: [-2.5, 2.2, -0.5], size: [0.45, 2.2, 5] as [number, number, number], color: '#67e8f9', emissive: '#0891b2', rot: [0.05, 0, 0.2] },
        { pos: [2.2, 2.5, -1.5], size: [0.4, 2.8, 5] as [number, number, number], color: '#a78bfa', emissive: '#6d28d9', rot: [0, 0.5, -0.15] },
        { pos: [0.3, 3.2, -1], size: [0.5, 2.5, 5] as [number, number, number], color: '#22d3ee', emissive: '#0e7490', rot: [0.1, 0.3, 0.08] },
        { pos: [-1, 2, -2.5], size: [0.35, 1.8, 5] as [number, number, number], color: '#c084fc', emissive: '#7c3aed', rot: [0, 0.8, -0.25] },
        { pos: [1.5, 1.5, 1.5], size: [0.3, 1.5, 5] as [number, number, number], color: '#67e8f9', emissive: '#0891b2', rot: [-0.1, 1.2, 0.12] },
      ].map((cr, i) => (
        <mesh
          key={`crystal-${i}`}
      ...
      <pointLight position={[0, 1.5, 2.5]} color="#c084fc" intensity={2} distance={6} />
